...
import { connectLive } from "@/lib/live"; // <-- add this
...
const ModuleRun: React.FC = () => {
  ...
  // NEW: live data from FastAPI WebSocket
  ...
  useEffect(() => {
    ...
    const t = tasks[idx];
    speak(t.instruction);
    if (t.duration) setTimerRunning(true);
  ...
  return (
    ...
    >
      ...
      {/* Live camera area (from FastAPI MJPEG) */}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70">
          <div className="bg-card rounded-xl p-6 w-[90%] max-w-md space-y-3">
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="secondary" onClick={() => onFailChoice("retry")}>
                Try Again
              </Button>
              <Button variant="secondary" onClick={() => onFailChoice("skip")}>
                I succeeded (skip)
              </Button>
      ...
      {showCongrats && (
        <div
          ...
          style={{ background: "hsl(var(--accent-progress))" }}
        >
